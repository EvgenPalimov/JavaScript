...
function handleClick(clickedButtonEvent) {
    ...
    const textOfButton = card.button.innerText;
...
function showMoreText(card) {
    ...
    let randomText1 = "Lorem ipsum dolor, sit amet consectetur adipisicing elit.";
    let randomText2 = "Eveniet, magni! Voluptates dolor inventore quam perferendis doloribus.";
    let randomText3 = "Qui quam blanditiis nemo repellendus ullam magni repellat."
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    switch (card.productName.innerText) {
        case "Product 1 name":
            card.productName.insertAdjacentHTML('afterend', `<div class="desc">${randomText1}</div>`);
            break
        case "Product 2 name":
            card.productName.insertAdjacentHTML('afterend', `<div class="desc">${randomText2}</div>`);
            break
        case "Product 3 name":
            card.productName.insertAdjacentHTML('afterend', `<div class="desc">${randomText3}</div>`);
            break
    }


    // Сделал под каждый товар свой текст. Единственное не понял, как избавить от дублирования кода?
